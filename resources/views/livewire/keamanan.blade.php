...
<button class="ibadah_katalog" style="float:right;">+</button>

<center>
    <h2>Keamanan</h2> 
    <form wire:submit="tambah_data_keamanan">
    <div class="konten">
        
        <input type="text" wire:model="nama" placeholder="Nama">
        <input type="text" wire:model="prodi" placeholder="Prodi">
        <input type="text" wire:model="ibadah" placeholder="Ibadah">
...
<hr>
<h1>Tampilkan data <select class="bulan" name="" id="">
...
document.querySelector('.ibadah_katalog').onclick = function (){
    pmt = prompt("Tambahkan katalog atau jenis data!");

   fetch("/katalog-ibadah", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content
        },
        body: JSON.stringify({ ibadah: pmt })
    })
    .then(res => res.json())
    .then(data => {
        if (data.valid) {
